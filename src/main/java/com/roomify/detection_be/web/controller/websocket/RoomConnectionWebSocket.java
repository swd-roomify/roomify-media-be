...
public class RoomConnectionWebSocket {
  @Autowired private RoomWSService roomSessionService;
  ...
  //
  //        UserJoinReq user =
  // UserJoinReq.builder().userId(userId).username(username).character(character).build();
  ...
  public void join(
      @Header String roomId, UserJoinReq user, SimpMessageHeaderAccessor headerAccessor) {
    log.info("Raw message: {}", user);
    log.info(
        "Received user details: userId={}, username={}, character={}",
        user.getUserId(),
        user.getUsername(),
        user.getCharacter());
    String sessionId = headerAccessor.getSessionId();
    ...
    roomSessionService.addSessionToRoom(roomId, sessionId, user);
    log.info(
        "User {} joined session {} with ID {} and character {}",
        user.getUsername(),
        sessionId,
        user.getUserId(),
        user.getCharacter());
