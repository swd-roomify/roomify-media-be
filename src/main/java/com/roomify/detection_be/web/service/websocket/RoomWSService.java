...
public class RoomWSService {
  ...
  @Autowired private HashOperations<String, String, UserWSRes> roomUserHashOperations;
  ...
  public void addSessionToRoom(String roomId, String sessionId, UserJoinReq user) {
    UserWSRes savedUser =
        new UserWSRes(user.getUserId(), user.getUsername(), user.getCharacter(), 400, 400);
  ...
  public void broadcastToUserByRoomId(String destination, String roomId) {
    messagingTemplate.convertAndSend(
        "/topic" + destination + "/" + roomId, getUsersByRoomId(roomId));
  }
  ...
  public void handleUserMove(String roomId, UserMoveReq userMoveReq, String sessionId) {
    log.info(
        "User is moving with info: {}, {}, {}",
        userMoveReq.getUserId(),
        String.valueOf(userMoveReq.getPositionX()),
        String.valueOf(userMoveReq.getPositionY()));
    if (roomUserHashOperations.hasKey(getRoomKey(roomId), getSessionKey(sessionId))) {
  ...
  public void handleUserChat(String roomId, UserChatReq user, String sessionId) {
    if (roomUserHashOperations.hasKey(getRoomKey(roomId), getUserKey(user.getUserId()))) {
      UserWSRes foundUser =
          roomUserHashOperations.get(getRoomKey(roomId), getSessionKey(sessionId));
      messagingTemplate.convertAndSend(
          "/chat/" + roomId,
          new UserChatWSRes(user.getUserId(), foundUser.getUsername(), user.getMessage()));
    }
  }
}
